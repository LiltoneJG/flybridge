...
- `workflow start --batch <file.json> --notify-terminal <Orca-handle>` は batch を永続化し、`batch_id` を返します。すべての item が開始失敗、orchestrated の終端、または single の作業報告に達したとき親 terminal に通知します。`workflow single-report <id> --outcome done|blocked --summary <text>` は稼働中の single で有効な queue request がないときだけ受け付け、workflow 自体は終了しません。親が通知後に single の結果を検証して終了します。`workflow batch status <batch-id>` で各 item と通知エラーを確認できます。停止した watcher は稼働中の item の supervisor が再起動し、親 terminal が無効なら未配達として残します。
...
- waiting request の昇格通知は SQLite に残るため、昇格中に observer が停止していても再起動後に配達できます。稼働中の workflow が resource 待ちになったら、supervisor は `queue.observer` や `--no-queue-observer` の指定にかかわらず observer を起動・再起動します。後続が待つ lease を有効な owner が `queue.lease_timeout_seconds` より長く保持したら、supervisor は idle の owner に後片付けと解放の確認を促します。有効な owner の lease は強制解放しません。
